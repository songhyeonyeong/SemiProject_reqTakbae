	...
	<div id="wrap">
		...
		<div id="subContainer">
			...
			<div class="contBox inner">
				...
				<div class="titNavi">
					 공고 <span>홈 &gt; 공고&gt; 상세보기</span>
					<!-- <p class="flo_left mb10"><a class="mbtn rd"  href="#fn_open" onclick="fn_open()">신고하기</a></p>user에게만 보임 -->
					<script>
		function fn_open() {
			<%if (loginUser != null) {%>
			   window.open('<%=request.getContextPath()%>/views/board/reportPop.jsp?gno=<%=num%>&title=<%=a.getG_TITLE()%>&name=<%=loginUser.getUser_name()%>&uno=<%=loginUser.getU_no()%>',
											'reportPop',
											'scrollbars=yes,menubar=no,toolbar=no,location=no,top=50,left=500,width=600,height=600');
					<%}%>
						}
					</script>
				...
				<div class="rtv of mt30">
					...
					<div class="flo_left wth300">
						...
						<p>
							<img
								src="<%=request.getContextPath()%>/upload/<%=img.getI_c_name()%>"
								alt="상품이미지" width="300" height="250">
						</p>
						<%
							} else {
						%>
						...
						%>
						<p class="font30 tcen">
							신청자 인원 <em class="font30 orange"><%=a.getCount()%></em>명
						</p>
					</div>
					<!--// flo_left E-->
					...
					<div class="flo_right wth850">
						...
						<table class="noticeWrite">
							...
							<%
								if (loginUser != null) {
							%>
							<p class="flo_right mb10">
								<a class="mbtn rd" href="#fn_open" onclick="fn_open()">신고하기</a>
							</p>
							<%
								} else {
							%>
							...
							<tbody id="money">
								...
								<tr>
									...
									<td colspan="3"><%=a.getG_CONTEXT()%></td>
								</tr>

								<script>
									var t = $('#money').children().eq(3)
											.children().eq(3).text();
									$('#money').children().eq(3).children().eq(
											3).text(Number(t).format());
								</script>
				...
				<div class="clear btnbox mt30 mb30">
					...
					%>
					<span><a class="mbtn gy wth60"
						href="<%=request.getContextPath()%>/updateBO?gongdiv=<%=gongdiv%>&&gno=<%=num%>">수정하기</a></span>

					<span><a class="mbtn rd wth60"
						href="<%=request.getContextPath()%>/deleteBoard.bo?gongdiv=<%=gongdiv%>&&gno=<%=num%>">삭제</a></span>
					<%
						}
					%>
					<%
						} else {
							if (!(loginUser.getUser_div().equals("관리자"))) {
					%>
							<span><a class="mbtn db wth60"
							href="<%=request.getContextPath()%>/ReqAnnouncement.bo?gongdiv=<%=gongdiv%>&&gno=<%=num%>">신청하기</a></span>
					<%
						} else {
					%>
							<a hidden></a>
					...
					%>
					<%-- href='<%=request.getContextPath()%>/selectList.bo?gongdiv=<%=gongdiv%>&&gno=<%=num%>' --%>
					<span><a class="mbtn bk wth60" href="#" onclick="history.back(-1)"
						>돌아가기</a></span>
