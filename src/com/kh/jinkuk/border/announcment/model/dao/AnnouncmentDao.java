...
public class AnnouncmentDao {
	...
	/*
	 * //당일공고 상세보기 public Announcment selectOneToday(Connection con, int num) {
	 * 
	 * PreparedStatement pstmt = null; ResultSet rset = null; Announcment a = null;
	 * 
	 * String query =prop.getProperty("selectOneToday"); try { pstmt =
	 * con.prepareStatement(query); pstmt.setInt(1, num);
	 * 
	 * rset = pstmt.executeQuery();
	 * 
	 * if(rset.next()) { a = new Announcment();
	 * 
	 * a.setUSER_ID(rset.getString("USER_ID"));
	 * a.setG_TITLE(rset.getString("G_TITLE"));
	 * a.setG_S_DATE(rset.getDate("G_S_DATE"));
	 ...
	 * a.setG_S_AREA(rset.getString("G_S_AREA"));
	 * a.setG_E_AREA(rset.getString("G_E_AREA"));
	 * a.setG_PRICE(rset.getInt("G_PRICE")); a.setG_SIZE(rset.getString("G_SIZE"));
	 * a.setG_DAY(rset.getDate("G_DAY"));
	 * a.setG_CONTEXT(rset.getString("G_CONTEXT"));
	 * a.setG_P_DIV(rset.getString("G_P_DIV"));
	 * 
	...
	public int plusCmoney(Connection con, InsertAnnouncment i, int gno) {
		PreparedStatement pstmt = null;
		int result = 0;

		String query = prop.getProperty("plusCmoney");
